...
def get_hopespot_files(hopespot_name):
    ...
    for audio_file, data in audio_data.items():
        ...
        audio_file_path = os.path.join(base_path, 'audio', audio_file_no_ext, audio_file)
        file_info = {
            'audio': url_for('static', filename=f'hopespots/{hopespot_name}/audio/{audio_file_no_ext}/{audio_file}', _external=True),
...
def get_popular_file(hopespot_name):
    ...
    audio_file_no_ext = os.path.splitext(most_popular_audio)[0]
    audio_file_path = os.path.join(base_path, 'audio', audio_file_no_ext, most_popular_audio)

    # Construct response for the most popular audio file
    response = {
        'audio': url_for('static', filename=f'hopespots/{hopespot_name}/audio/{audio_file_no_ext}/{most_popular_audio}', _external=True),
